...
const rawSchema = z.object({
  tracker: unknownRecord.optional(), polling: unknownRecord.optional(), workspace: unknownRecord.optional(), state: unknownRecord.optional(), hooks: unknownRecord.optional(), agent: unknownRecord.optional(), logs: unknownRecord.optional(),
}).passthrough();
...
export function buildConfig(workflow: WorkflowDefinition, repoPath: string, overrides: { stateRoot?: string } = {}): ServiceConfig {
  ...
  const tracker = raw.tracker ?? {}; const polling = raw.polling ?? {}; const workspace = raw.workspace ?? {}; const state = raw.state ?? {}; const hooks = raw.hooks ?? {}; const agent = raw.agent ?? {}; const logs = raw.logs ?? {};
