...
3. **Push the version tag** from the bump commit to trigger the release workflow:
   ```bash
   git tag v0.x.0-preview.1
   git push origin v0.x.0-preview.1
   ```
   The hyphen-detection logic in `release.yml` routes the publish to the npm `next` dist-tag — so `npm install -g <pkg>` still resolves to the previous stable, and `npm install -g <pkg>@next` opts into the preview.
4. **Iterate**. Bug reports and fixes against `0.x.0-preview.1` land on the `release/0.x.0` branch (or merge-forward from `main` if the fix is generally applicable). Each iteration bumps to `0.x.0-preview.N` and pushes a new tag to trigger the workflow.
5. **Cut the stable release.** When the preview has been validated, bump to `0.x.0` (no hyphen) and push the `v0.x.0` tag. The workflow publishes under `latest`. The GitHub release is no longer marked pre-release.
...
git tag v0.x.y
git push origin v0.x.y   # triggers the release workflow
