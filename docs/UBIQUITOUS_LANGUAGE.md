...
The content posted as a tracker comment for human review before an agent is dispatched. In v1,
the plan content is the rendered prompt — the same text the agent would receive as its task
instruction. A future extension will allow the plan to be generated by the agent itself (an
agent-proposed plan). Avoid calling this a "proposal" or "preview" — use "plan" consistently.
Reference: `docs/plan-handshake.md`

### Plan Handshake

The two-phase dispatch sequence enabled by `agent.plan_first: true`: post a plan comment →
wait for operator approval → dispatch the agent. The handshake is per-attempt: each new
attempt requires a fresh approval cycle. Use "plan handshake" as the noun for the feature;
"plan-first mode" for the runtime state when `agent.plan_first: true`.
Reference: `docs/plan-handshake.md`, `packages/conduit/src/domain/types.ts`

### Plan Pending

An issue whose plan has been posted but whose approval label (`plan-approved` or
`plan-rejected`) has not yet been seen by the orchestrator. Tracked in
`PersistedState.planPending` keyed by `issueId`. An issue in plan-pending state is excluded
from dispatch until the operator acts. Avoid: "awaiting review" (too vague), "blocked" (that
term is reserved for tracker blocker relations).
Reference: `packages/conduit/src/domain/types.ts`
...
`agent.plan_first: true`, the additional event `on_plan_posted` fires when
Conduit posts the plan comment. Each event maps to an optional
`TrackerWriteAction` (`comment` and/or `transition_to`).
Reference: `packages/conduit/src/domain/types.ts:22`
